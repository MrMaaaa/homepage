<template>
  <div class="content">
    ...
    <div class="grid" id="grid">
      <div class="grid-item" :class="'grid-item' + item" v-for="item in itemTotal" :data-index="item" :key="item">{{ item }}</div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.outer = document.querySelector('#grid');
    // 使node节点伪数组能使用数组的方法
    let items = Array.prototype.slice.call(
      document.querySelectorAll('#grid .grid-item')
    );
    items.forEach((value, index) => {
      this.itemsPos.push({
        element: value,
        top: Math.round(Utils.getStyle(value, 'top')),
        left: Math.round(Utils.getStyle(value, 'left'))
      });
    });
    //获取元素宽高
    this.itemSize.width = parseInt(Utils.getStyle(items[0], 'width'));
    this.itemSize.height = parseInt(Utils.getStyle(items[0], 'height'));
  ...
  methods: {
    ...
    itemMoveHandle(evt) {
      ...
      if (this.isMove) {
        let { mouseX, mouseY } = this.getMousePos(evt);
        this.clickItem.style.left =
          mouseX - this.clickItem.offsetWidth / 2 + 'px';
        this.clickItem.style.top =
          mouseY - this.clickItem.offsetHeight / 2 + 'px';
    ...
    outerMouseUp(evt) {
      ...
      if (this.isMove) {
        ...
        if (target.classList.contains('grid-item')) {
          ...
          // 获取拖动之后元素的位置
          this.afterPos.top = Math.round(Utils.getStyle(this.clickItem, 'top'));
          this.afterPos.left = Math.round(
            Utils.getStyle(this.clickItem, 'left')
          );
          // 获取拖动元素对应的元素的位置
          ...
          if (coveredItem) {
            ...
            let coveredItemPos = {
              top: Math.round(Utils.getStyle(coveredItem, 'top')),
              left: Math.round(Utils.getStyle(coveredItem, 'left'))
            };
            ...
            $(coveredItem).animate(
              ...
              },
              {
                complete: function() {
                  ...
                  that.itemsPos[that.coveredItemIndex].left =
                    that.beforePos.left;
                }
              }
            );
    ...
    getMousePos(evt) {
      return {
        mouseX: Math.round(
          evt.clientX - this.outer.getBoundingClientRect().left
        ),
    ...
    getCoveredItem() {
      ...
      this.itemsPos.forEach(function(value, index) {
        ...
        //如果相交
        if (
          Math.abs(clickCenterX - thisCenterX) <
            thisWidth / 2 + clickWidth / 2 &&
          Math.abs(clickCenterY - thisCenterY) <
            thisHeight / 2 + clickHeight / 2
        ) {
          // 相交
          // 判断相交区域
          ...
          if (intersectDirection === 2 || intersectDirection === 4) {
            let resultPosLeftTop =
              value.left > that.afterPos.left
                ? {
                    left: value.left,
                    top: value.top
                  }
            ...
            thisArea =
              Math.abs(resultPosLeftTop.left - resultPosRightBottom.left) *
              Math.abs(resultPosLeftTop.top - resultPosRightBottom.top);
          } else if (intersectDirection === 1 || intersectDirection === 3) {
            let resultPosRightTop =
              value.left < that.afterPos.left
                ? {
                    left: value.left + that.itemSize.width,
                    top: value.top
            ...
            thisArea =
              Math.abs(resultPosRightTop.left - resultPosLeftBottom.left) *
              Math.abs(resultPosRightTop.top - resultPosLeftBottom.top);
          }
          if (thisArea >= maxArea) {
            maxArea = thisArea;
            maxIndex = index;
